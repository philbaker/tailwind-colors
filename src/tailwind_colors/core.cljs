...
(def custom-colors
  (get (js->clj (js/JSON.parse (str (fs/readFileSync "colors.json")))) "colors"))

(defn normalize-colors [coll]
...
(def project-colors
  (merge default-colors (walk/keywordize-keys (normalize-colors custom-colors))))

(def project-colors-rgb (into {} (map (fn [[k v]] [k (utils/rgb-format-css (utils/hex-to-rgb v))]) project-colors)))
...
(defn hex-to-class
  ...
  [hex-code]
  (name (key-from-value hex-code project-colors)))

(defn rgb-to-class
  "Get class name based on rgb value"
  [rgb-code]
  (name (key-from-value rgb-code project-colors-rgb)))

(defn class-to-hex
  "Get hex code based on class name"
  [class-name]
  ((keyword class-name) project-colors))

(defn class-to-rgb
  "Get rgb code based on class name"
  [class-name]
  ((keyword class-name) project-colors-rgb))
